...
function Login({ onLogin }) {
  ...
  const [step, setStep] = useState(1); // 1: credentials, 2: biometric
  ...
  const handleCredentialsSubmit = async (e) => {
    ...
    if (username && password) {
      ...
      setError('');
      try {
        const res = await fetch('http://localhost:8000/authentication/verify-credentials/', {
          method: 'POST',
  ...
  const handleAutoLogin = async (imageData) => {
    ...
    setError('');
    try {
      const res = await fetch('http://localhost:8000/authentication/verify-biometric/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          username: username,
      ...
      if (res.ok && data.verified) {
        ...
        try {
          const monitoringResponse = await fetch('http://localhost:8000/authentication/start-monitoring/', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        setError(data.error || 'Biometric verification failed. Please try again.');
        setLoading(false);
      }
    } catch (err) {
      setError('Network error. Please check your connection.');
