...
export const LoginWithBiometric = () => {
  ...
  const isVerifiedWithCredentials = useSelector((state) => state.user.isVerifiedWithCredentials)
      ...
      <div className={styles.card}>
        <div className={styles.header}>
          ...
          <p className={styles.subtitle}>
            Please verify your identity using {getBiometricTypeLabel(biometricType).toLowerCase()}
          </p>
        ...
        <div className={styles.photoComparison}>
          <div className={styles.photoSection}>
            ...
            <div className={styles.photoFrame}>
              {nationalIdPhotoUrl ? (
                <img src={nationalIdPhotoUrl || "/placeholder.svg"} alt="National ID" className={styles.photo} />
              ) : (
          ...
          <div className={styles.photoSection}>
            ...
            <div className={`${styles.photoFrame} ${cameraActive ? styles.photoFrameActive : ""}`}>
              {cameraActive ? (
                <div style={{ position: "relative", width: "100%", height: "100%" }}>
                  <video ref={videoRef} autoPlay className={styles.video} />
                  {countdown > 0 && (
                    <div className={`${styles.captureStatus} ${styles.captureStatusActive}`}>
                      Capturing in {countdown}s
                    </div>
                  )}
                  {countdown === 0 && loading && (
                    <div className={`${styles.captureStatus} ${styles.captureStatusActive}`}>Processing...</div>
                  )}
                </div>
              ) : capturedImage ? (
                <img src={capturedImage || "/placeholder.svg"} alt="Live Capture" className={styles.photo} />
              ) : (
                <div className={styles.noPhoto}>
                  <div className={styles.cameraIcon}>📷</div>
                  Ready to capture
        ...
        {/* Match Result */}
        {showMatchResult && (
          <div className={matchSuccess ? styles.matchSuccess : styles.matchFailure}>
            {matchSuccess ? <>✅ Face Match Successful</> : <>❌ Face Does Not Match</>}
          </div>
        )}
        ...
        {!loading && error && !matchSuccess && started && (
          <div className={styles.retryMessage}>
            Your face does not match with the national ID photo. Please ensure good lighting and try again.
          </div>
        )}
        ...
        {/* Error Message */}
        {error && !showMatchResult && <div className={styles.errorMessage}>{error}</div>}

        {/* Security Note */}
        <div className={styles.securityNote}>
          <p className={styles.securityText}>
            🔒 Your biometric data is encrypted and processed securely. This verification ensures exam integrity.
          </p>
        </div>
      </div>
    </div>
